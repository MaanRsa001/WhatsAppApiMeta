...
public class PhoenixMozambiqueWhatsAppServiceImpl implements PhoenixMozambiqueWhatsAppService{
	...
	@Override
	public String mozambiqueWebhookRes(WebhookReq webhookReq) {
		try {

			log.info("Mozambique webhookRes--> request: ");
				...
				if (count > 0) {
					...
					if (list.size() <= 0 || StringUtils.isNotBlank(replyMsgId)) {
						...
						if (replyIsJob.equalsIgnoreCase("N")) {
							...
							WAWatiReq sended_res = watiSer.sendSessMsgMozambique(msgM, waid);
						...
						if (isJob.equalsIgnoreCase("N") || nxtMsgId.equals("MZB001")) {   //COM001
	...
	private String sendLanguageMsg(WebhookReq req) {
		...
		try {
			String message ="*Phoenix Mozambique Corporate Limited*\n\nPlease choose your language";
			...
			String meta_message_api=cs.getwebserviceurlProperty().getProperty("mozambique.message.api");
			String meta_message_api_auth=cs.getwebserviceurlProperty().getProperty("mozambique.message.auth");
