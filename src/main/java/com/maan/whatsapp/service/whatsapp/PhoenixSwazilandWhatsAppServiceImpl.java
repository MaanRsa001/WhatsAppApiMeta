...
public class PhoenixSwazilandWhatsAppServiceImpl implements PhoenixSwazilandWhatsAppService{
	...
	@Override
	public String swazilandWebhookRes(WebhookReq webhookReq) {
		
		try {

			log.info("Swaziland webhookRes--> request: ");
				...
				if (count > 0) {
					...
					if (list.size() <= 0 || StringUtils.isNotBlank(replyMsgId)) {
						...
						if (replyIsJob.equalsIgnoreCase("N")) {
							...
							WAWatiReq sended_res = watiSer.sendSessMsgSwaziland(msgM, waid);
	...
	private String sendLanguageMsg(WebhookReq req) {
		...
		try {
			String message ="*Phoenix Swaziland Corporate Limited*\n\nPlease choose your language";
			...
			String meta_message_api=cs.getwebserviceurlProperty().getProperty("swaziland.message.api");
			String meta_message_api_auth=cs.getwebserviceurlProperty().getProperty("swaziland.message.auth");
