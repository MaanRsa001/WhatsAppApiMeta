...
public class PhoenixBoatswanaWhatsAppServiceImpl implements PhoenixBoatswanaWhatsAppService{
...
private Logger log = LogManager.getLogger(getClass());
	...
	@Override
	public String boatswanaWebhookRes(WebhookReq webhookReq) {
		try {

			log.info("Boatswana webhookRes--> request: ");
				...
				if (count > 0) {
					...
					if (list.size() <= 0 || StringUtils.isNotBlank(replyMsgId)) {
						...
						if (replyIsJob.equalsIgnoreCase("N")) {
							...
							WAWatiReq sended_res = watiSer.sendSessMsgBoatswana(msgM, waid);
						...
						if (isJob.equalsIgnoreCase("N") || nxtMsgId.equals("SWZ001")) {   //COM001
							...
							if (NumberUtils.isCreatable(reply) || userButtonReply.equalsIgnoreCase(previousMenu)
								...
								if (userReply == 0) {
									nxtMsgId = "BTW001";  //COM001
								} else {
								//	Map<String, Object> map = waChatRepo.getNextMsgId(wdd.getParentmessageid(),userReply);
									Map<String, Object> map = getNextMsgId(wdd.getParentmessageid(),
											userReply);
							...
							if (isJob.equalsIgnoreCase("N") || nxtMsgId.equals("BTW001")) { //COM001
							...
							if (reqDetList.size() > 0) {
								...
								if ("Y".equalsIgnoreCase(isResSaveApi) && "Y".equalsIgnoreCase(isApiCall)
									...
									isValid = watiApiCall.sendValidationMsg(okhttp, body, commonurl, msgurl, fileurl,
									...
									isValid = watiApiCall.sendIsResYnMsg(okhttp, body, commonurl, button_msg_url,
								...
								if (isDocUpl.equalsIgnoreCase("Y") && StringUtils.isNotBlank(data)
									...
									watiApiCall.sendDocValidationMsg(okhttp, body, commonurl, msgurl, fileurl, auth,
								...
								if ("Y".equals(reqDet.getIsprocesscompleted())) {
									Flux.fromIterable(waidList).map(i -> watiSer.callSendSessionMsg(i, msgid))
											.subscribeOn(Schedulers.boundedElastic()).subscribe();
								}
	...
	private String sendLanguageMsg(WebhookReq req) {
		...
		try {
			String message ="*Phoenix Boatswana Corporate Limited*\n\nPlease choose your language";
			...
			String meta_message_api=cs.getwebserviceurlProperty().getProperty("boatswana.message.api");
			String meta_message_api_auth=cs.getwebserviceurlProperty().getProperty("boatswana.message.auth");
